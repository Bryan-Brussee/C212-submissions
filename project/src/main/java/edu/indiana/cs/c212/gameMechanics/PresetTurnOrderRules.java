...
public class PresetTurnOrderRules implements Rules {
	protected final Board board;
	private Queue<Player> players;

	public PresetTurnOrderRules(Board board, LinkedList<Player> turnOrder, Player red, Player blue) {
		this.board = board;
		players = new LinkedList<Player>();
		
		
		for (int i = 0; i < turnOrder.size(); i++) {
			players.add(turnOrder.get(i));
